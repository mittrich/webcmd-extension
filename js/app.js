...
function startTime() {
    ...
    // Lấy đúng thứ, ngày, tháng, năm
    var dayOfWeek = today.getDay(); // 0-6, dùng cho tên thứ
    var dayOfMonth = today.getDate(); // 1-31, ngày trong tháng
    var month = today.getMonth() + 1; // 1-12, nhớ cộng 1
    var year = today.getFullYear();

    // Format số <10 thành 0x
    h = checkTime(h);
    m = checkTime(m);
    s = checkTime(s);
    dayOfMonth = checkTime(dayOfMonth);
    month = checkTime(month);

    var d_s = dayOfWeekAsString(dayOfWeek);

    document.getElementById('time').innerHTML =
        `Bây giờ là ${h}:${m}:${s} ⏲️ ${d_s} ngày ${dayOfMonth}/${month}/${year} 📅`;
...
function welcomeMsg() {
    ...
    if (curHr < 12) {
        document.getElementById('welcome').innerHTML = `☀️ Chào buổi sáng ${USERNAME}.`;
    } else if (curHr < 18) {
        document.getElementById('welcome').innerHTML = `🌤️ Chào buổi chiều ${USERNAME}.`;
    } else {
        document.getElementById('welcome').innerHTML = `🌙 Chào buổi tối ${USERNAME}.`;
    }
}
